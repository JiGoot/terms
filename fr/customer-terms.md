...
* **"Fournisseur"** fait référence à un restaurant, un magasin, pharmacie ou tout autre fournisseur répertorié sur la Plateforme.
...
### 5. Substitution des Articles de Commande

En utilisant la Plateforme, le client et le fournisseur reconnaissent que certains articles d'une commande peuvent être remplacés conformément aux préférences de substitution sélectionnées. Les préférences de substitution incluent :

* **`best_match` :** La plateforme peut sélectionner automatiquement un article similaire dans la même catégorie et sous-catégorie, classé par proximité de prix ou selon les historiques de substitution.
* **`selection` :** La plateforme tentera de fournir les alternatives spécifiques choisies par le client.
* **`skip` :** Les articles peuvent être laissés non fournis si indisponibles.
* **`cancel` :** La commande entière peut être annulée si un article est indisponible.

Si aucune alternative valide n'est disponible pour un article défini sur `best_match` ou `selection`, l'article peut être ignoré (`skip`). Les fournisseurs peuvent être évalués sur la précision de leur stock, et des mesures de performance peuvent être appliquées pour garantir que les articles visibles par les clients sont réellement disponibles. En passant une commande ou en gérant le stock, toutes les parties acceptent cette politique de substitution.
